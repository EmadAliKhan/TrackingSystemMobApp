...
export default function NotificationScreen() {
  ...
  const fetchNotificationData = useCallback(async () => {
    try {
      ...
      setNotifications(appNotifs.sort((a, b) => (a.type === "new_task" ? -1 : 1)));
    } catch (error) {
  ...
  const renderItem = ({ item }: { item: AppNotification }) => (
    ...
    >
      <View style={[s.iconCircle, { backgroundColor: item.type === 'new_task' ? '#3B82F620' : '#F59E0B20' }]}>
        <Ionicons 
      ...
      <View style={s.textContainer}>
        <View style={s.row}>
          ...
          <Text style={[s.time, item.type === "new_task" && { color: "#3B82F6", fontWeight: 'bold' }]}>
            {item.time}
          </Text>
        </View>
    ...
    <View style={s.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl refreshing={refreshing} onRefresh={() => {setRefreshing(true); fetchNotificationData();}} tintColor="#fff" />
          }
          ListEmptyComponent={
